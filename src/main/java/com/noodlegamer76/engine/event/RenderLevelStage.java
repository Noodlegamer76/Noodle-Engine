...
public class RenderLevelStage {
    ...
    public static void renderLevelStage(RenderLevelStageEvent event) {
        ...
        if (event.getStage() == RenderLevelStageEvent.Stage.AFTER_SKY) {
            ...
            PhysicsSpace physicsSpace = PhysicsEngine.getInstance().spaceManager.getPhysicsSpace();
            Collection<PhysicsRigidBody> bodies = physicsSpace.getRigidBodyList();
            List<PhysicsRigidBody> threadSafe = new ArrayList<>(bodies);
            for (PhysicsRigidBody body : threadSafe) {
                poseStack.pushPose();
