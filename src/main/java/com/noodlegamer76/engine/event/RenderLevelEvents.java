...
import com.noodlegamer76.engine.client.renderer.gltf.MaterialBatch;
    ...
    public static void renderLevel(RenderLevelStageEvent event) {
        ...
        for (Map.Entry<McMaterial, Map<GltfVbo, List<RenderableBuffer>>> batchEntry : batch.getBuffers().entrySet()) {
            ...
            for (Map.Entry<GltfVbo, List<RenderableBuffer>> vboEntry : vbos.entrySet()) {
                ...
                for (RenderableBuffer buffer: vboEntry.getValue()) {
                    vbo.drawWithShader(buffer.getModelViewMatrix(), RenderSystem.getProjectionMatrix(), material.getShaderRef().shader);
                }
            }
        }


        GlbRenderer.getBatch().clear();
        VertexBuffer.unbind();
    }
}
